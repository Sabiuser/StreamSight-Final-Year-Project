...
function UserAvatar({ userId, showStatus = false }) {
  const c = hashColor(userId || "");
  const initials =
    userId?.replace("user_", "U").toUpperCase().slice(0, 2) || "??";
  return (
    <Tooltip title={userId} placement="top">
      ...
      >
        <Box>
          ...
          <Box display="flex" alignItems="center" gap={1}>
            <Typography
              sx={{ color: "#475569", fontSize: 13, fontWeight: 500 }}
            >
              Active Sessions:
            </Typography>
            <Typography
        ...
        >
          ...
          <tbody>
            <AnimatePresence mode="popLayout">
              {filteredEvents.map((e, i) => {
                const cfg =
                  EVENT_CONFIG[e.event_type] || EVENT_CONFIG.page_view;
                const devCfg = DEVICE_CONFIG[e.device] || DEVICE_CONFIG.desktop;
                return (
                  <motion.tr
                    key={e.id || i}
                  ...
                  >
                    ...
                    <td style={{ padding: "14px 16px" }}>
                      <Typography
                        sx={{ color: "#f8fafc", fontWeight: 600, fontSize: 12 }}
                      >
                        {e.country}
                      </Typography>
                    </td>
            ...
            <Box py={1}>
              <Box display="flex" alignItems="center" gap={2} mb={3}>
                ...
                <Box>
                  ...
                  <Typography fontSize="0.75rem" color="#10b981">
                    ● Currently Active
                  </Typography>
                </Box>
              </Box>
              <Box display="flex" flexDirection="column" gap={2}>
                {[
                  {
                    label: "Current Event",
                    value: EVENT_CONFIG[selectedUser.event_type]?.label,
                    icon: <Public sx={{ fontSize: 16 }} />,
                    color: EVENT_CONFIG[selectedUser.event_type]?.color,
                  },
                  {
                    label: "Location",
                  ...
                  {
                    label: "Path",
                    value: selectedUser.page,
                    icon: <Visibility sx={{ fontSize: 16 }} />,
                  },
                  {
                    label: "Time",
                    value: formatTime(selectedUser.timestamp),
