      ...
      {count > 4 && (
        <div
          style={{
            ...
            background: "#EEF3FF", color: "#185FA5", border: "2px solid white",
...
function Carousel({ onEventSent }) {
  ...
  const [current, setCurrent] = useState(0);
  const [direction, setDirection] = useState(1);
  // CHANGE 1: Track per-product "added to cart" state in carousel
  const [carouselAdded, setCarouselAdded] = useState({});
  ...
  const bs = BADGE_MAP[p.badge] || BADGE_MAP["Classic"];
  const isAddedToCart = carouselAdded[p.id];

  return (
    <div
      style={{
        position: "relative", borderRadius: 16, overflow: "hidden", height: 300,
        marginBottom: 32, background: "linear-gradient(135deg, #0f172a 0%, #1e293b 100%)",
        boxShadow: "0 8px 32px rgba(15,23,42,0.18)",
      }}
    >
      <div
        style={{
          ...
          backgroundSize: "300% 100%", animation: "ss-shimmer 4s linear infinite", zIndex: 5,
        }}
      />
          ...
          <div style={{ display: "flex", alignItems: "center", width: "100%", padding: "0 56px 0 48px", gap: 36 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                {/* CHANGE 1: One-time Add to Cart — disables after first click */}
                <motion.button
                  whileHover={!isAddedToCart ? { scale: 1.03 } : {}}
                  whileTap={!isAddedToCart ? { scale: 0.97 } : {}}
                  onClick={() => {
                    if (isAddedToCart) return;
                    sendEvent({ event_type: "add_to_cart", product_id: p.id, price: p.price, page: `/product/${p.id}`, category: p.category });
                    addToCart(p);
                    setCarouselAdded((prev) => ({ ...prev, [p.id]: true }));
                    toast.success("🛒 Added to cart!");
                    onEventSent();
                  }}
                  style={{
                    display: "flex", alignItems: "center", gap: 6, padding: "10px 22px",
                    borderRadius: 10,
                    border: isAddedToCart ? "1px solid rgba(16,185,129,0.4)" : "1px solid rgba(255,255,255,0.2)",
                    background: isAddedToCart ? "rgba(16,185,129,0.15)" : "rgba(255,255,255,0.08)",
                    color: isAddedToCart ? "#10B981" : "#E2E8F0",
                    fontSize: 13, fontWeight: 500,
                    cursor: isAddedToCart ? "default" : "pointer",
                    transition: "all 0.3s",
                    opacity: isAddedToCart ? 0.85 : 1,
                  }}
                >
                  {isAddedToCart
                    ? <><CheckCircle style={{ fontSize: 16 }} /> Added ✓</>
                    : <><AddShoppingCart style={{ fontSize: 16 }} /> Add to Cart</>
                  }
                </motion.button>
              </div>
      ...
      {[{ icon: <NavigateBefore />, dir: -1, side: "left" }, { icon: <NavigateNext />, dir: 1, side: "right" }].map((btn) => (
        <motion.button
          ...
          style={{
            ...
            border: "1px solid rgba(255,255,255,0.18)", color: "#CBD5E1", cursor: "pointer",
            display: "flex", alignItems: "center", justifyContent: "center", zIndex: 4, transition: "background 0.2s",
          }}
        >
          {btn.icon}
        </motion.button>
...
function ProductCard({ product, onEventSent }) {
  ...
  const [wishlist, setWishlist] = useState(false);
  const [cartFlash, setCartFlash] = useState(false);
  // CHANGE 1: One-time cart state per product card
  const [addedToCart, setAddedToCart] = useState(false);
  // CHANGE 1: One-time cart state for the dialog button
  ...
  // CHANGE 1: handleCart is one-time — disables button after first click
  const handleCart = () => {
  ...
  // CHANGE 2: Checkout = cart review page (navigate with cart context)
  const handleCheckout = () => {
    action("checkout");
    navigate("/payment", { state: { product, mode: "checkout" } });
    onEventSent();
  };

  // CHANGE 2: Buy Now = skip cart, direct to payment
  const handleBuy = () => {
    action("purchase");
    navigate("/payment", { state: { product, skipCart: true } });
    onEventSent();
  };
  ...
  return (
    <>
      <motion.div
        ...
        transition={{ type: "spring", stiffness: 280, damping: 22 }}
        style={{
          background: "#FFFFFF", borderRadius: 14, border: "1px solid #E8EDF3",
          boxShadow: "0 2px 8px rgba(15,23,42,0.06)", overflow: "hidden",
          height: "100%", display: "flex", flexDirection: "column", transition: "box-shadow 0.2s",
        }}
      >
        <div style={{ position: "relative", height: 200, overflow: "hidden", background: "#F8FAFC", flexShrink: 0 }}>
          <motion.img
            src={product.img} alt={product.name} whileHover={{ scale: 1.07 }}
            transition={{ duration: 0.38 }}
          ...
          {cartQty > 0 && (
            <motion.div
              ...
              style={{
                position: "absolute", top: 46, right: 10, width: 32, height: 32, borderRadius: "50%",
                background: cartFlash ? "#1D9E75" : "#EEF3FF", border: `2px solid ${cartFlash ? "#1D9E75" : "#378ADD"}`,
          ...
          {lowStock && (
            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "6px 10px", background: "rgba(255,255,255,0.94)" }}>
              ...
              <LinearProgress
                variant="determinate" value={(product.stock / 20) * 100}
                sx={{ height: 4, borderRadius: 2, backgroundColor: "#FEF3C7", "& .MuiLinearProgress-bar": { backgroundColor: "#F59E0B", borderRadius: 2 } }}
              />
            </div>
          )}
        </div>
        ...
        <div style={{ padding: "14px 16px 16px", flex: 1, display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 10 }}>
            <span style={{ fontSize: 18, fontWeight: 800, color: "#059669", fontVariantNumeric: "tabular-nums" }}>${product.price.toFixed(2)}</span>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 7, marginTop: "auto" }}>
            <div style={{ display: "flex", gap: 7 }}>
              <Tooltip title="View product details">
                <motion.button
                  ...
                  onClick={handleView} className="ss-action-btn"
              ...
              {/* CHANGE 1: Add to Cart is one-time — shows Added ✓ and disables after click */}
              <Tooltip title={addedToCart ? "Already in cart" : "Add to cart"}>
                <motion.button
                  whileHover={!addedToCart ? { scale: 1.03 } : {}}
                  whileTap={!addedToCart ? { scale: 0.97 } : {}}
                  onClick={handleCart} className="ss-action-btn"
                  style={{
                    flex: 1, display: "flex", alignItems: "center", justifyContent: "center",
                    gap: 5, height: 34, borderRadius: 8,
                    border: addedToCart ? "1px solid #A7F3D0" : "1px solid #BFDBFE",
                    background: addedToCart ? "#ECFDF5" : "#EFF6FF",
                    color: addedToCart ? "#059669" : "#185FA5",
                ...
                >
                  {addedToCart
                    ? <><CheckCircle style={{ fontSize: 14 }} /> Added ✓</>
            ...
            <div style={{ display: "flex", gap: 7 }}>
              {/* CHANGE 2: Checkout = review cart items then pay */}
              <Tooltip title="Review your cart items and proceed to payment">
                <motion.button
                  whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                  onClick={handleCheckout} className="ss-action-btn"
              ...
              {/* CHANGE 2: Buy Now = skip cart, go directly to payment */}
              <Tooltip title="Skip cart — go directly to payment now">
                <motion.button
                  whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                  onClick={handleBuy} className="ss-action-btn"
      ...
      >
        ...
        <DialogActions sx={{ p: 2.5, gap: 1, borderTop: "1px solid #F1F5F9" }}>
          {/* CHANGE 1: Dialog Add to Cart is also one-time */}
          <motion.button
            whileHover={!dialogAdded ? { scale: 1.02 } : {}}
            whileTap={!dialogAdded ? { scale: 0.98 } : {}}
            onClick={() => {
              if (dialogAdded) return;
              handleCart();
              setDialogAdded(true);
            }}
            style={{
              display: "flex", alignItems: "center", gap: 6, padding: "9px 18px", borderRadius: 9,
              border: dialogAdded ? "1px solid #A7F3D0" : "1px solid #BFDBFE",
              background: dialogAdded ? "#ECFDF5" : "#EFF6FF",
              color: dialogAdded ? "#059669" : "#185FA5",
              fontSize: 13, fontWeight: 600, cursor: dialogAdded ? "default" : "pointer",
              transition: "all 0.3s",
            }}
          >
            {dialogAdded
          ...
          {/* CHANGE 2: Buy Now in dialog = skip cart too */}
          <motion.button
          ...
          >
            <FlashOn style={{ fontSize: 16 }} /> Buy Now — Skip Cart
          </motion.button>
        </DialogActions>
      </Dialog>
    </>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 16, marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <motion.button
                whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                onClick={() => {
                  if (cartItems.length > 0) navigate("/payment", { state: { cartItems, mode: "checkout" } });
                  else toast.info("Cart is empty — add some products first!");
                }}
                style={{
                  ...
                  color: totalItems > 0 ? "white" : "#64748b",
                  border: `1px solid ${totalItems > 0 ? "#10b981" : "#e2e8f0"}`,
                  borderRadius: 24, padding: "6px 16px", cursor: "pointer", fontWeight: 700, fontSize: 13,
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <Tooltip title="Simulate user leaving website (triggers bounce event in Kafka)">
              <motion.button
                ...
                onClick={handleLeave} className="ss-action-btn"
            ...
            <Tooltip title="Simulate fraud — price $9999 triggers Isolation Forest ML detection">
              <motion.button
                whileHover={{ scale: 1.02, y: -1 }} whileTap={{ scale: 0.97 }}
                onClick={handleHack} className="ss-action-btn"
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 24, flexWrap: "wrap", alignItems: "center" }}>
          {CATEGORIES.map((cat) => {
            ...
            return (
              <motion.button
                key={cat} whileHover={{ y: -1 }} whileTap={{ scale: 0.96 }}
                onClick={() => setFilter(cat)} className="ss-cat-chip"
                style={{
                  padding: "7px 18px", borderRadius: 24, fontSize: 13,
                  fontWeight: active ? 700 : 500, border: active ? "none" : "1px solid #E2E8F0",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 20 }}>
          {filtered.map((product, i) => (
            <motion.div
              ...
              transition={{ delay: i * 0.05, duration: 0.35 }} style={{ height: "100%" }}
            >
              <ProductCard product={product} onEventSent={handleEventSent} />
            </motion.div>
          ))}
        ...
        <div style={{ marginTop: 32, padding: "16px 20px", borderRadius: 12, background: "#EFF6FF", border: "1px solid #BFDBFE", display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <p style={{ color: "#1E40AF", fontSize: 13, margin: 0, lineHeight: 1.6 }}>
            ...
            <strong>Buy Now</strong> skips the cart and goes directly to payment.{" "}
            <strong>Checkout</strong> takes you to cart review first.{" "}
            <strong>Add to Cart</strong> can only be done once per product — click again to go to checkout.{" "}
            <strong>Simulate Attack</strong> triggers ML anomaly detection.
          </p>
        </div>
      </div>
    </>
